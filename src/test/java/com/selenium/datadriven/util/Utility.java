...
public class Utility extends TestBase{
	...
	public static boolean  isTestRunable(String TestCaseName, Xls_Reader xls){
		
		int row = xls.getRowCount(Constants.TESTCASE_SHEET);
		System.out.println(row);
		
		for(int rNum =2; rNum<row; rNum++){
			String xlData = xls.getCellData("Test Cases", Constants.TCID_COL, rNum);
			if(xlData.equalsIgnoreCase(TestCaseName)){
				String runMode = xls.getCellData("Test Cases", Constants.RUNMODE_COL, rNum); 
		...
		return false; // BY default it will return false if testcase is not found
	}
	...
	public static Object logIn_Application() throws InterruptedException{
